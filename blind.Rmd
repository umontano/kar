...
#ceg  <- read.csv("https://raw.githubusercontent.com/umontano/kar/master/ceg.csv")
library(readr)
...
#
library(ggplot2)
ggplot(ceg, aes(CE_sindistractibilidad, Calificacion, col=Sexo) ) +
geom_point() +
geom_smooth(method="lm", se=T, fullrange=TRUE) +
         annotate("point",
#                   xmin=0, xmax=7,
                    x=mean(ceg$CE_sindistractibilidad), 
                    y=mean(ceg$Calificacion) , 
...
ceg %>% select(nivelactividad : conjunto_atencion) %>% dplyr::filter(!is.na(nivel_actividad)) %>%  -> traits 
